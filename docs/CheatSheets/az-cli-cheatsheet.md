...
```sh  
az account set -s "azure-cloud"

# Get a list of subscriptions for the logged in account.
az account list -o table 

# To view all the Azure subscription names and IDs for a specific Microsoft account,
az account list --query "[?user.name=='testuser@example.com'].{Name:name, ID:id, Default:isDefault}" --output Table
...
az webapp log tail --name 'feedback-api-app' --resource-group 'test-rg-dev'

# Download a web app's log history as a zip file.
az webapp log download --name 'feedback-api-app' --resource-group 'test-rg-dev' --log-file webapp_303029911.zip
```
